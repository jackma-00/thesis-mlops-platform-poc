...
def train():
    ...
    with mlflow.start_run(run_name="train-model-xgboost") as run:
        ...
        # Saving reference test data
        X_test = pd.DataFrame(X_test)
        y_test = pd.DataFrame(y_test)

        X_test.to_csv("../data-layer/data/monitoring/reference_X_test.csv", index=False, header=False)
        y_test.to_csv("../data-layer/data/monitoring/reference_y_test.csv", index=False, header=False)
